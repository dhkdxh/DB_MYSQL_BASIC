 ...
 * INSERT, DELETE, UPDATE, SELECT -> DML을 DAO클래스에서 수행함
 * 비즈니스 로직(Service)에서 DAO호출해서 데이터 처리를 맡김
 *
 * DAO를 사용하는 이유
 * 1. 관심사 분리 (자신이 맡은 역할만 집중해서 담당하며 수행함)
 * 2. 유지 보수 용이  => DB구조가 바뀌어도 DAO만 바꾸면 됨
 * 3. 재사용성 증가시킬 수 있음 => 여러 서비스에서 동일한 DAO를 호출가능하여 재사용성을 높일 수 있음
 * </p>
 *
 * <p>
 * <strong>주요 기능:</strong>
